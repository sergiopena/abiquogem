class Abiquo::VirtualDatacenter < Abiquo
	...
	def list_vappliances
		url = self._getlinks self.vdc,'virtualappliances'
		xml = self._httpget(url)
		vapps = []
		xml['virtualAppliance'].each { |r|
			vapps << r["id"][0]
		}
		return vapps
	end

	def get_vapp_by_id(id)
		url = "api/cloud/virtualdatacenters/#{self.id}/virtualappliances/#{id}"
		xml = self._httpget(url)

		return xml
	end

	def create(	enterpriselink, iddatacenter )
